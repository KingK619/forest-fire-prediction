...
input_df = pd.read_csv("./data/csv/apiInput.csv")
...
my_df.to_csv('./data/csv/predictions.csv')
...
for lat, lon,name in cooridnates:
    ...
    for l in range(length):
        ffmc=0.0
        date=datetime.datetime.fromtimestamp( data[l]['dt']).strftime("%d %b %Y")
        temp = (data[l]['temp']['max'] + data[l]['temp']['min'])/2
        ...
        # Feed the 'paramList' to the model. The prediction done by the model is stored in 'prediction'.
        prediction = random.randint(0, 100)                            # assigned random values for now.
        # Append the tuple into 'finalList'
        futurePredictList.append((lat,lon,name,date,prediction))
    
# #convert data to CSV for heatmap 

column_names = ['latitude', 'longitude','name','date','prediction']
data = np.array(futurePredictList)
my_df = pd.DataFrame(data, columns=column_names)
my_df.to_csv('./data/csv/graphData.csv')
